...
import { extend, GqlSubscriptionService, SubscriptionConfig } from '@nestjs/graphql';
...
export class ApolloDriver extends ApolloBaseDriver {
  ...
  public async start(options: ApolloDriverConfig) {
    ...
    if (options.definitions && options.definitions.path && options.schema) {
      await this.graphQlFactory.generateDefinitions(printSchema(options.schema), options);
    }

    await this.registerServer(options);

    if ((options.installSubscriptionHandlers || options.subscriptions) && options.schema) {
      const subscriptionsOptions: SubscriptionConfig = options.subscriptions || { 'subscriptions-transport-ws': {} };
