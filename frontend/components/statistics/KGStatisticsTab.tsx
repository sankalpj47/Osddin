...
import {
  columnKGTop10ByBetweenness,
  columnKGTop10ByCloseness,
  columnKGTop10ByDegree,
  columnKGTop10ByEigenvector,
  columnKGTop10ByPageRank,
} from '@/lib/data';
import { kgStatisticsGenerator } from '@/lib/graph';
...
const CENTRALITY_CONFIGS = [
  { value: 'degree', label: 'By Degree', dataKey: 'degree', data: 'top10ByDegree', columns: columnKGTop10ByDegree },
  {
    value: 'betweenness',
    label: 'By Betweenness',
    dataKey: 'betweenness',
    data: 'top10ByBetweenness',
    columns: columnKGTop10ByBetweenness,
  },
  {
    value: 'closeness',
    label: 'By Closeness',
    ...
    columns: columnKGTop10ByCloseness,
  },
  {
    value: 'eigenvector',
    ...
    columns: columnKGTop10ByEigenvector,
  },
  {
    value: 'pagerank',
    label: 'By PageRank',
    ...
    columns: columnKGTop10ByPageRank,
  },
] as const;

/**
 * KGStatisticsTab - Statistics view for Knowledge Graph using KGStore
 * Lazy-loaded component that displays network metrics and centrality analysis
 */
export function KGStatisticsTab() {
  const networkStatistics = useKGStore(state => state.networkStatistics);
  const statisticsComputed = useKGStore(state => state.statisticsComputed);
  ...
  // Compute statistics lazily on first tab open
  useEffect(() => {
    if (!statisticsComputed && sigmaInstance) {
      const graph = sigmaInstance.getGraph();
      const stats = kgStatisticsGenerator(graph);
      setNetworkStatistics({
        totalNodes: graph.order,
        totalEdges: graph.size,
        ...stats,
      });
      useKGStore.setState({ statisticsComputed: true });
    }
  }, [statisticsComputed, sigmaInstance, setNetworkStatistics]);
  ...
  return (
    <div className='mx-4 my-6 flex flex-col gap-4 pb-6'>
      ...
      <Card className='pb-6'>
        <CardHeader>
          <CardTitle>Node Degree Distribution</CardTitle>
                    ...
                    <TabsContent value='chart'>
                      {hasData ? (
                        <ChartContainer className='safari-only-svg-fix max-h-[400px] w-full' config={{}}>
                          <BarChart data={data}>
                            <CartesianGrid strokeDasharray='3 3' />
                            <XAxis dataKey='label' label={{ value: 'Node', position: 'insideBottom', offset: -5 }} />
                            <YAxis
                            ...
                            />
                            <ChartTooltip content={<ChartTooltipContent labelKey='Node' />} />
      ...
      <Card className='pb-6'>
        <CardHeader>
          ...
          <CardDescription>
            Cumulative distribution of edge scores <br /> Shows the number of edges with a score greater than or equal
            to a certain value.
          </CardDescription>
        </CardHeader>
        <CardContent className='flex justify-center'>
          {networkStatistics.edgeScoreDistribution ? (
            <ChartContainer className='safari-only-svg-fix max-h-[400px] w-full' config={{}}>
              <AreaChart data={networkStatistics.edgeScoreDistribution}>
                <CartesianGrid strokeDasharray='3 3' />
                <XAxis dataKey='score' label={{ value: 'Edge Score', position: 'insideBottom', offset: -5 }} />
                <YAxis
                  dataKey='count'
                  tickCount={7}
                  label={{ value: 'Number of Edges', angle: -90, position: 'insideLeft' }}
                />
